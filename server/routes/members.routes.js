...
r.get('/', validate(listMembersQuerySchema, 'query'), asyncHandler(async (req, res) => {
  res.json({ items: await svc.listMembers(req.query) });
}));

// upsert by name (docs/05-business-rules.md §3.2) — existing name -> 200 the
// existing row, brand-new name -> 201 the newly-created one.
r.post('/', validate(upsertMemberSchema), asyncHandler(async (req, res) => {
  const { member, created } = await svc.upsertMember(req.body.name);
  res.status(created ? 201 : 200).json(member);
}));

r.patch('/:id', validate(idParamSchema, 'params'), validate(updateMemberSchema), asyncHandler(async (req, res) => {
  res.json(await svc.updateMember(req.params.id, req.body));
}));

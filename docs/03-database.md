...
## 3.7 Migration 005/006 — ปฏิทินรวมของทีม (.ics feed ต่อสมาชิก)

`005_calendar.sql` สร้างตารางนี้ครั้งแรกแบบ Microsoft Graph OAuth (`access_token_enc`/`refresh_token_enc`) แต่ยังไม่เคยมีข้อมูลจริง (ฟีเจอร์ยังไม่เปิดใช้) จึง `006_calendar_ics.sql` ดร็อปแล้วสร้างใหม่เป็นรูปแบบลิงก์ .ics แทน (ไม่ต้องตั้งค่า Azure AD) — ไฟล์: `server/db/migrations/006_calendar_ics.sql`
...
หนึ่งแถวใน `calendar_connections` = ลิงก์ .ics ที่สมาชิกหนึ่งคน publish ไว้จาก Outlook เอง (`member_id UNIQUE` — เชื่อมต่อใหม่แทนที่ของเดิม ไม่สร้างซ้ำ, ลิงก์เข้ารหัส AES-256-GCM ก่อนเก็บเพราะเป็น bearer secret — `server/utils/crypto.js`) ส่วน `calendar_events` เป็นแคชที่ `server/services/calendar.service.js`'s `pollAllConnections()` (ticked จาก `server/index.js` ทุก `CALENDAR_POLL_MINUTES` นาที) ดึงลิงก์มา parse ด้วย `server/utils/ics.js` แล้วลบ+insert ใหม่ทั้งหมดทุกรอบ (wipe-and-reinsert ต่อ connection ในช่วง "วันนี้ −1 วัน ถึง +14 วัน" — ไม่ทำ incremental diff) ดู `docs/04-api.md` §12

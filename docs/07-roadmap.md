...
- [x] เตือนก่อนปิดงานไม่ครบ — ลากการ์ด (หรือ bulk move) เข้าคอลัมน์ `is_done` ขณะขั้นตอนยังไม่ครบ/ไม่มีขั้นตอนเลย ขึ้น confirm เตือนก่อน 1 ครั้ง ไม่บล็อก (`board.view.js`'s `incompleteSubtasksWarning`, ใช้ร่วมกันทั้ง drag เดี่ยวและ bulk move) ดู `docs/05-business-rules.md` §4.3
- [x] หน้าภาพรวมทีม (สำหรับหัวหน้า) — ตาราง "ภาพรวมทีม" บน Dashboard: งานที่ถืออยู่/ค้างไม่ขยับ/เกิน-ใกล้ครบ SLA แยกรายคน เรียงคนที่น่าเป็นห่วงที่สุดขึ้นก่อน (`dashboard.view.js`'s `teamOverviewRows`) frontend ล้วน ใช้ข้อมูลจาก bootstrap ที่มีอยู่แล้ว
- [x] ปฏิทินรวมของทีม (Outlook/M365 sync) — แต่ละสมาชิกเชื่อมต่อ Outlook ของตัวเองผ่าน Microsoft Graph OAuth (`calendar_connections`/`calendar_events` tables, `005_calendar.sql`) ระบบ poll ดึงอีเวนต์เป็นรอบทุก `CALENDAR_POLL_MINUTES` นาที (ไม่ใช้ webhook เพราะไม่มี public HTTPS endpoint), token เข้ารหัส AES-256-GCM ก่อนเก็บ (`server/utils/crypto.js`), OAuth `state` เซ็นด้วย HMAC ป้องกัน CSRF โดยไม่ต้องมีระบบ session (`server/utils/oauthState.js`) — หน้า `#/calendar` (`public/js/views/calendar.view.js`) แสดงรายสัปดาห์ พร้อม filter รายสมาชิก, เชื่อมต่อ/ยกเลิกทำที่หน้า Members ดู `docs/04-api.md` §12 และ `docs/09-deployment.md` (ขั้นตอนตั้งค่า Azure AD)
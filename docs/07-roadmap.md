...
- [x] ปฏิทินรวมของทีม (.ics feed ต่อสมาชิก) — แต่ละสมาชิก publish ปฏิทิน Outlook ของตัวเองเป็นลิงก์ `.ics` แล้ววางไว้ที่หน้า Members ไม่ต้องตั้งค่า Azure AD/OAuth ใดๆ (`calendar_connections`/`calendar_events` tables, `006_calendar_ics.sql`) ระบบ poll ดึง+parse ลิงก์เป็นรอบทุก `CALENDAR_POLL_MINUTES` นาที ด้วย parser ที่เขียนเอง (`server/utils/ics.js` — รองรับ RRULE พื้นฐาน DAILY/WEEKLY/MONTHLY), ลิงก์เข้ารหัส AES-256-GCM ก่อนเก็บเพราะเป็น bearer secret (`server/utils/crypto.js`) — หน้า `#/calendar` (`public/js/views/calendar.view.js`) แสดงรายสัปดาห์ พร้อม filter รายสมาชิก ดู `docs/04-api.md` §12 และ `docs/09-deployment.md` §4.5 (ขั้นตอน publish ปฏิทิน)
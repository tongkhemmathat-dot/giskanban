...
4. การ์ดในคอลัมน์ `pauses_sla = 1` (ปัจจุบันคือ **Waiting Vendor**) พัก SLA ชั่วคราว — ไม่นับว่าเกินกำหนด/ใกล้ครบระหว่างที่อยู่ในนั้น
   - เข้าคอลัมน์ → บันทึก `sla_paused_at = now`
   - ออกจากคอลัมน์ → เลื่อน `sla_due_at` ไปข้างหน้าเท่ากับเวลาที่พักไว้ (`now - sla_paused_at`) แล้วเคลียร์ `sla_paused_at`
   - ป้องกันทีมโดนนับว่าเกิน SLA ทั้งที่รอฝั่งผู้ให้บริการภายนอกอยู่
...
| `paused` | อยู่ในคอลัมน์ `pauses_sla = 1` | ⚪ `⏸ พัก SLA` |
